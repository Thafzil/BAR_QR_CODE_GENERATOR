...
def MongoDB():
    ...
    client = MongoClient("mongodb+srv://sarda2t:Mobilecomputing123%40@cluster0.tlktwhi.mongodb.net/test")
...
def save_to_db():
    ...
    new_images = [image for image in images if (image['name'], image['type']) not in [(existing_image['name'], existing_image['type']) for existing_image in existing_images]]
    # if there are any new images, add them to the database
    if new_images:
        # if the document already exists, update it
        if existing_doc:
            images_records.update_one({'user': user}, {'$push': {'images': {'$each': new_images}}})
        ...
        response = jsonify({'message': f"{len(new_images)} new image/s uploaded to the database successfully for your future reference"})
        response.status_code = 200
    else:
        response = jsonify({'message': 'This image is already uploaded to the database'})
        response.status_code = 200
...
def delete_from_db():
    ...
    if existing_doc:
        existing_image = next((image for image in existing_doc['images'] if image['name'] == image_name and image['type'] == image_type), None)
        if existing_image:
            ...
            response = jsonify({'message': 'This image not found in your database'})
            response.status_code = 404
    return response
...
def delete_all_images():
    ...
    if existing_doc:
        existing_doc['images']=[]
        images_records.replace_one({'user': user}, existing_doc)
        response = jsonify({'message': 'All image/s deleted from your database'})
        response.status_code = 200
    return response
...
def get_from_db():
    ...
    matching_docs = list(images_records.find({"user": request.json.get('user')}))
    ...
    for value in merged_images:
        ...
        if value['type']=='bar':
            ...
            images.append({'name':text, 'image':img_data, 'type': value['type']})
        if value['type']=='qr':
            text = value['name']
            if not text:
                return Response(status=400)
...
def generate_barcodes():
    ...
    df = pd.read_csv(request.files.get('csv_file'), header=None, index_col=None)
