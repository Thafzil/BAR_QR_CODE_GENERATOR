...
export class AppService {
  ...
  signup(name:string, username:string,password:string, password_confirm:string){
    const formData = new FormData();
  ...
  isLoggedIn():boolean{
    return localStorage.getItem('user') !== null && localStorage.getItem('user_name') !== null; 
  }
  ...
  deleteFromDB(imageName, imageType){
    const body ={'user':localStorage.getItem('user'), 'image_name':imageName, 'image_type':imageType}
    const headers = new HttpHeaders({ 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' });
    return this.http.post(remote_api_url+'/delete_from_db', body, {headers});
  }

  deleteAllFromDB(){
    const body ={'user':localStorage.getItem('user')}
    const headers = new HttpHeaders({ 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' });
    return this.http.post(remote_api_url+'/delete_all_images', body, {headers});
  }

  getSavedCount(){
    const body ={'user':localStorage.getItem('user')}
    const headers = new HttpHeaders({ 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' });
    return this.http.post(remote_api_url+'/get_saved_count', body, {headers});
  }

  generateBarcode(text: string) {
    const body = { text };
    const headers = new HttpHeaders({ 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' });
    return this.http.post(remote_api_url+'/barcode', body, { headers, responseType: 'blob' });
  }
  ...
  generateQRcode(text: string) {
    ...
    return this.http.post(remote_api_url+'/qrcode', body, { headers, responseType: 'blob' });
  }
